...
BEGIN
  OPEN reminders FOR 
    SELECT P.REMINDER_TITLE, to_char(P.REMINDER_TIME, 'DD-MM-YYYY HH24:MI:SS'), P.REMINDER_ID
...
BEGIN
  OPEN reminders FOR 
    SELECT P.REMINDER_TITLE, to_char(P.REMINDER_TIME, 'DD-MM-YYYY HH24:MI:SS'), P.REMINDER_ID, to_char(P.REMINDER_INTERVAL, 'DD-MM-YYYY HH24:MI:SS')
    FROM DIARY_REMINDER P
...
BEGIN
  ...
  VALUES (
    ...
    r_USER_ID, 
    r_REMINDER_INTERVAL,
    TO_DATE(r_REMINDER_TIME, 'DD-MM-YYYY HH24:MI:SS'),
    r_REMINDER_TITLE 
    );
END;
...
create or replace 
PROCEDURE CHECK_USERNAME_AND_PASSWORD(
    u_USERNAME VARCHAR2,
    u_USER_PASSWORD VARCHAR2,
    o_USER_ID OUT NUMBER,
    o_PREMIUM OUT CHAR
)
AS
BEGIN
    SELECT u.user_id, u.premium
    INTO o_USER_ID, o_PREMIUM
    FROM diary_users u
    WHERE u.username = u_USERNAME AND u.user_password = u_USER_PASSWORD;
EXCEPTION
...
BEGIN
  ...
  DELETE FROM DIARY_IMAGE D WHERE D_DIARY_ID = D.DIARY_ID;
END;
/
...
/
create or replace 
PROCEDURE INSERT_IMAGE (DIARY_ID INTEGER, PLACE_IN_TEXT INTEGER, IMAGE_PATH VARCHAR2,NEW_IMAGE_ID OUT INTEGER)
AS
BEGIN
  NEW_IMAGE_ID := diary_image_seq.NEXTVAL;
  INSERT INTO DIARY_IMAGE I VALUES (new_image_id, diary_id, image_path, SYSDATE, place_in_text);
END;
/
